  ...
  return (
    <div className="space-y-8">
      <header className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Backtests & Diagnostics</h1>
      ...
      <Card className="p-6 space-y-4 border-2">
        <div className="flex flex-wrap items-center gap-4">
          <label className="text-sm uppercase tracking-wide text-muted-foreground">Symbol</label>
          <select
      ...
      <Card className="p-6 space-y-4 border-2">
        <header className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-foreground">Signals ({signals.length})</h2>
          {signalsLoading && <span className="text-sm text-muted-foreground">Loading…</span>}
        </header>
        {signalsError && (
              ...
              <tbody>
                {signals.slice(0, 25).map((signal) => (
                  <tr key={signal.id} className="border-b border-border/40">
                    <td className="py-2 pr-4 font-mono text-foreground">{signal.symbol}</td>
                    <td className="py-2 pr-4 text-foreground">{signal.signal_type}</td>
                    <td className="py-2 pr-4 text-foreground">{Math.round(signal.strength)} / 100</td>
      ...
      <section className="grid md:grid-cols-2 gap-6">
        <Card className="p-6 space-y-3 border-2">
          <header className="flex items-center justify-between">
            ...
            {marketLoading && <span className="text-sm text-muted-foreground">Loading…</span>}
          </header>
          {marketError && (
            <div className="text-red-600 text-sm">
        ...
        <Card className="p-6 space-y-3 border-2">
          <header className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-foreground">Backtest Summary</h2>
            {backtestLoading && <span className="text-sm text-muted-foreground">Loading…</span>}
          </header>
          {backtestError && (
            <div className="text-red-600 text-sm">
          ...
          {!backtestError && backtestSummary && (
            <div className="space-y-2 text-sm text-foreground">
              ...
              {backtestSummary.notes && (
                <p className="text-xs text-muted-foreground">Notes: {backtestSummary.notes}</p>
              )}
            </div>
          )}
        </Card>
      ...
      <Card className="p-6 space-y-3 border-2">
        <header className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-foreground">Latest Signal JSON</h2>
          {latestSignal && (
            <span className="text-sm text-muted-foreground">{selectedSymbol} current signal snapshot</span>
          )}
        </header>
        {latestSignal ? (
